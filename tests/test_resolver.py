...
class TestParametersResolver:
    ...
    @pytest.mark.parametrize("template, context, expected", [
        ({"key1": "${{ a }}", "key2": "${{ b }}"}, {"a": "x", "b": "y"}, {"key1": "x", "key2": "y"}),
        ({"name": "Hello, ${{ name }}!"}, {"name": "Alice"}, {"name": "Hello, Alice!"}),
        ({"key1": "${{ a }}", "key2": {"key3": "${{ b }}"}}, {"a": 1, "b": 2}, {"key1": 1, "key2": {"key3": 2}}),
    ])
    def test_dict_input(self, template: dict[str, Any], context: dict[str, Any], expected: dict[str, Any]) -> None:
        resolver = ParametersResolver()
        assert resolver.resolve(template, context) == expected
    ...
    @pytest.mark.parametrize("template, context", [
        ...
        pytest.param("${{ valid_var.invalid_field }}", {"valid_var": {'valid_field': 1}}, marks=pytest.mark.xfail()),
