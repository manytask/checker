...
from ..course.schedule import CourseSchedule, Task
from ..utils.manytask import get_score
from ..utils.print import print_info
from ..utils.files import filename_match_patterns
from ..utils.glab import GITLAB, MASTER_BRANCH, get_private_project, get_students_projects, get_student_file_link
...
EXPORT_IGNORE_FILE_PATTERNS: list[str] = []  # ['__init__.py']
EXPORT_FILE_EXTENSIONS: list[str] = ['.py', '.cfg', '.yml', '.yaml']
SIMILARITY_MIN_THRESHOLD = 0.5

PARALLEL_WORKERS = 8


def _get_ended_solutions(
        course_schedule: CourseSchedule,
        course_driver: CourseDriver,
) -> Generator[tuple[Task, list[str]], None, None]:
    ...
    if group_names:
        ...
        for group_name in group_names:
            if group_name in course_schedule.groups:
                group = course_schedule.groups[group_name]
        ...
        now_ = datetime.now()  # TODO: check timezone
        ended_tasks = course_schedule.get_tasks(enabled=True)
        ended_tasks = [task for task in ended_tasks if task.group.second_deadline > now_ - END_TIMEDELTA]
        ended_tasks = [task for task in ended_tasks
                       if now_ - NO_ENDED_TIMEDELTA < task.group.second_deadline < now_ + END_TIMEDELTA]
    ...
    for task in ended_tasks:
        ...
        relative_task_paths = {
            *[i.relative_to(task.private_dir) for i in task.private_dir.glob('**/*')
              if i.is_file() and i.suffix in EXPORT_FILE_EXTENSIONS],
            *[i.relative_to(task.public_dir) for i in task.public_dir.glob('**/*')
              if i.is_file() and i.suffix in EXPORT_FILE_EXTENSIONS],
        }
        task_files = [
            str(i) for i in relative_task_paths
            if not filename_match_patterns(i, [*EXPORT_IGNORE_FILE_PATTERNS, *task.config.test_files])
        ]
...
def download_solutions(
    ...
    for task, task_files in _get_ended_solutions(course_schedule, course_driver):
        ...
        for filename in task_files:
            ...
            print_info('downloading solved solutions...', color='grey')
            _, _, students_codes = get_all_students_code(
                private_project, students_projects, task.name, remote_path, solved=True
            )

            print_info('writing unique solutions...', color='grey')
            print_info(f'students_codes {len(students_codes)}', color='grey')
            total_solutions_count, unique_solutions_count = write_unique_solutions(
                students_codes, remote_path, local_path
            )
    ...
    try:
        ...
        if solved:
            ...
            if len(project_users_filtered_by_name) == 0:
                # print_info(f'No users in project {full_student_project.name}:{student_project_id}
                # with name {full_student_project.name}', color='red')
                return None
            if len(project_users_filtered_by_name) != 1:
                print_info(
                    f'Multiple users in filter {full_student_project.name}: {project_users_filtered_by_name}',
                    color='orange'
                )
            ...
            score = get_score(report_base_url, task_name, user_id)
            if score is None:
                return None
        ...
        if student_file == solution_template_file:
            # logging.debug('File is equal to template. Skipping: %s',
            # get_student_file_link(student_project_name, path_to_file))
            return None
        elif student_file == canonical_file:
            print_info(
                'File is equal to canonical solution: ',
                get_student_file_link(gitlab_url, students_group_name, student_project_name, path_to_file)
            )
            return student_file
        else:
...
def get_all_students_code(
        private_project: gitlab.v4.objects.GroupProject,
        students_projects: list[gitlab.v4.objects.GroupProject],
        task_name: str,
        path_to_file: str,
        solved: bool = True,
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        ...
        future_to_author = {
            executor.submit(
                get_file_data, private_project.id, project.id, project.name, task_name, path.as_posix(), solved=solved
            ): project.name
    ...
    with local_path.open('w') as file_:
        for dump_hash, codes in unique_solutions_code.items():
            print(
                ...
                *[
                    f'# {author[:20]:<{20}} \t{get_student_file_link(gitlab_url, students_group_name, author, path)}'
                    for author in unique_solutions_authors[dump_hash]
                ],
                '#' * 60,
                file=file_, sep='\n', end='\n'
            )
